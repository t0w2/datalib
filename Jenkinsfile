node {
    ...
    stage('Status Checking') {
        if (env.CHANGE_ID) {
            withCredentials([usernameColonPassword(credentialsId: 'Github', variable: 'credentials')]) {
                SHA = sh(
                    script: "curl https://${credentials}@eos2git.cec.lab.emc.com/api/v3/repos/wangw5/datalib/pulls/${CHANGE_ID} 2> /dev/null | python -c \"import sys, json; print json.load(sys.stdin)['head']['sha']\"",
                    returnStdout: true
                ).trim()
                STATUS = sh(
                    script: "curl https://${credentials}@eos2git.cec.lab.emc.com/api/v3/repos/wangw5/datalib/commits/${SHA}/status 2> /dev/null | python -c \"import sys, json; print json.load(sys.stdin)['state']\"",
                    returnStdout: true
                ).trim()
                //STATUS = sh(
                //    script: "curl https://${credentials}@eos2git.cec.lab.emc.com/api/v3/repos/wangw5/datalib/commits/${SHA}/status 2> /dev/null | python -c \"import sys, json; checks = json.load(sys.stdin); num = len(checks['statuses']); print 'No checks found.' if (num <= 1) else ''; print ', '.join([checks['statuses'][i]['context'] + ' is ' + checks['statuses'][i]['state'] for i in range(num) if not (checks['statuses'][i]['state'] == 'success' or checks['statuses'][i]['context'] == 'continuous-integration/jenkins/pr-merge')])\"",
                //    returnStdout: true
                //).trim()
                if (STATUS != "success") {
    ...
    stage('Rebase') {
        if (env.CHANGE_ID) {
            withCredentials([usernameColonPassword(credentialsId: 'Github', variable: 'credentials')]) {
                SOURCE_BRANCH = sh(
                    script: "curl https://${credentials}@eos2git.cec.lab.emc.com/api/v3/repos/wangw5/datalib/pulls/${CHANGE_ID} 2> /dev/null | python -c \"import sys, json; print json.load(sys.stdin)['head']['ref']\"",
                    returnStdout: true
                ).trim()
                TARGET_BRANCH = sh(
                    script: "curl https://${credentials}@eos2git.cec.lab.emc.com/api/v3/repos/wangw5/datalib/pulls/${CHANGE_ID} 2> /dev/null | python -c \"import sys, json; print json.load(sys.stdin)['base']['ref']\"",
                    returnStdout: true
                ).trim()
